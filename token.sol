...
contract PAXTokenReserve is IERC20, Owned, usingOraclize {
    ...
    mapping(uint256 => uint256) public closingAccounts;
    ...
    uint256 public timestampOfNextMonth;
    ...
    bool public pendingOracle = false;
    ...
    function transferFrom(address from, address to, uint value) public returns (bool success) {
        ...
        payDemurrage(from);
        payDemurrage(to);
        if (value <= balanceOf(from) && value <= allowance(from, to)) {
            // Minimum Balance
            if (transacted[to][currentMonth] == false) {
                monthsLowestBalance[to][currentMonth] = _balances[to];
            }
    ...
    function mint(address account, address refferer) public {
        ...
        supply = supply.add(600000000000);
        _balances[worldTreasuryAddress] = _balances[worldTreasuryAddress].add(45500000000);
